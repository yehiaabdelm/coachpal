<script lang="ts">
	...
	const fetchOrganization = async (): Promise<
		| {
				name: string;
		  }
		| undefined
	> => {
		try {
			const res = await fetch(`${PUBLIC_API_URL}/organizations`, {
				credentials: 'include'
			});
	...
	let userData = $state(fetchOrganization());
</script>
...
>
	<p class="mt-4 gap-2 text-lg font-medium md:block">Organization</p>
	<p class="text-muted-foreground text-sm md:block">Update your organization settings</p>
</header>
...
{:then value}
	<form class="flex flex-1 flex-col space-y-6 overflow-y-auto p-4 pt-0">
		...
		<section class="space-y-2">
			<div class="flex w-1/2 gap-4 *:flex *:flex-1 *:flex-col *:gap-1.5">
				<div>
