...
export const InvitationEmail: React.FC<BaseEmailProps> = ({
    firstName = 'Yehia',
    invitedBy = 'Ali El-tawil',
    inviteUrl = 'https://coachpal.app',
}) => (
    <Html>
        <Head />
        <Preview>{invitedBy} has invited you to train with them on CoachPal.</Preview>
        <Body style={main}>
            <Container style={container}>
                <Img
                    ...
                    style={logo}
                />
                {/* <Heading style={h1}>Join {invitedBy} on CoachPal</Heading> */}
                <Text style={text}> Hey {firstName}</Text>
                <Text style={text}>
                    <span style={{ fontWeight: "bold" }}>{invitedBy}</span>  is inviting you to train with them on CoachPal. </Text>
                <Text style={text}>
                    You'll get personalized training programs, track your progress, and stay connected with your coach every step of the way.</Text>
                {inviteUrl && (
                    <>
